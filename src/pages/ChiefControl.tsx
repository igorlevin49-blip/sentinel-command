...
function MetricCard({
  title, value, suffix, icon: Icon, trend, trendUp, accent,
}: {
...
export default function ChiefControl() {
  ...
  return (
    <AppLayout title="Оперативный контроль качества">
      {/* Top KPIs */}
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        <MetricCard title="Покрытие смен" value={controlDashboardStats.shiftCoverage} suffix="%" icon={Users} trend="+2% к пр. нед." trendUp accent="bg-primary/10 text-primary" />
        <MetricCard title="Опоздания" value={controlDashboardStats.lateShifts} icon={Clock} accent="bg-warning/10 text-warning" />
        <MetricCard title="Пропущено смен" value={controlDashboardStats.missedShifts} icon={AlertTriangle} accent="bg-destructive/10 text-destructive" />
        <MetricCard title="Обходы выполн." value={controlDashboardStats.patrolCompletion} suffix="%" icon={Route} trend="+4%" trendUp accent="bg-success/10 text-success" />
        <MetricCard title="Нарушения перс." value={controlDashboardStats.personnelViolations} icon={ShieldAlert} trend="-2 к пр. нед." trendUp accent="bg-warning/10 text-warning" />
        <MetricCard title="Активных объектов" value={controlDashboardStats.activeObjectsCount} icon={Gauge} accent="bg-primary/10 text-primary" />
      </div>
              ...
              <BarChart data={slaByObject} layout="vertical" margin={{ left: 20 }}>
                ...
                <Bar dataKey="sla" radius={[0, 4, 4, 0]} name="SLA %">
                  {slaByObject.map((entry, index) => (
                    <Cell key={index} fill={entry.sla >= entry.target ? 'hsl(152, 60%, 40%)' : 'hsl(38, 92%, 50%)'} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ...
          <div className="p-4">
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={incidentsByObjectForControl} layout="vertical" margin={{ left: 20 }}>
                ...
                <YAxis type="category" dataKey="object" stroke="hsl(215, 15%, 50%)" fontSize={11} tickLine={false} axisLine={false} width={130} />
                <Tooltip contentStyle={tooltipStyle} />
                <Bar dataKey="open" stackId="a" fill="hsl(0, 72%, 51%)" name="Открытые" />
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <div className="rounded-lg border border-border bg-card">
          <div className="flex items-center justify-between border-b border-border px-5 py-4">
            ...
            <Badge variant="warning" className="text-[10px] px-1.5 py-0">{lateShifts.length}</Badge>
          </div>
          <div className="divide-y divide-border">
            {lateShifts.length > 0 ? lateShifts.map((shift) => (
              <div key={shift.id} className="flex items-center justify-between px-5 py-3 transition-colors hover:bg-muted/30">
                ...
                <Badge variant="destructive" className="text-[10px] px-1.5 py-0">{shift.violations} наруш.</Badge>
              </div>
            )) : (
        ...
        <div className="rounded-lg border border-border bg-card">
          <div className="flex items-center justify-between border-b border-border px-5 py-4">
            ...
            <Badge variant="default" className="text-[10px] px-1.5 py-0">{activePatrols.length}</Badge>
          </div>
          <div className="divide-y divide-border">
              ...
              return (
                <div key={patrol.id} className="px-5 py-3 transition-colors hover:bg-muted/30">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-mono text-xs text-foreground">{patrol.completedCheckpoints}/{patrol.checkpoints}</span>
                  </div>
