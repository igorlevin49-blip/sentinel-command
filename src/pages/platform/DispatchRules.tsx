...
export default function DispatchRules() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    let err;
    if (editId) {
      ({ error: err } = await supabase.from('dispatch_rules').update(payload).eq('id', editId));
    } else {
  ...
  async function toggleActive(id: string, current: boolean) {
    await supabase.from('dispatch_rules').update({ is_active: !current }).eq('id', id);
    fetchRules();
  }
          ...
          <form onSubmit={handleSubmit} className="rounded-lg border border-border bg-card p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div>
                ...
                <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Маршрутизация по зоне А" />
              </div>
              <div>
              ...
              <div className="sm:col-span-2">
                <label className="text-xs text-muted-foreground mb-1 block">
                  Условие (JSON)
                  {conditionJsonError && <span className="ml-2 text-destructive">{conditionJsonError}</span>}
                </label>
                <Textarea
                  rows={4}
                  value={form.condition_json}
                  onChange={(e) => setForm({ ...form, condition_json: e.target.value })}
                  placeholder='{"severity": "critical", "zone": "A"}'
                  className={`font-mono text-xs ${conditionJsonError ? 'border-destructive' : ''}`}
                />
              </div>
              <div className="sm:col-span-2">
                <label className="text-xs text-muted-foreground mb-1 block">
                  Действие (JSON)
                  {actionJsonError && <span className="ml-2 text-destructive">{actionJsonError}</span>}
                </label>
                <Textarea
                  rows={4}
                  value={form.action_json}
                  onChange={(e) => setForm({ ...form, action_json: e.target.value })}
                  placeholder='{"assign_to_org": "uuid", "notify_roles": ["dispatcher"]}'
                  className={`font-mono text-xs ${actionJsonError ? 'border-destructive' : ''}`}
                />
              </div>
