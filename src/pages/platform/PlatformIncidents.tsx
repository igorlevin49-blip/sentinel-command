...
const statusFlow: Record<IncidentStatus, IncidentStatus | null> = {
  created: 'accepted',
...
export default function PlatformIncidents() {
  ...
  async function advanceStatus(inc: Incident) {
    ...
    if (error) {
      setActionError(error.code === '42501' ? 'Нет прав для изменения статуса' : error.message);
    } else {
      setSelected(null);
      fetchIncidents();
    }
  ...
  async function handleAssign(inc: Incident) {
    ...
    const { error } = await supabase
      .from('incidents')
      .update({ assigned_to: assignTo.trim() })
      .eq('id', inc.id);

    if (error) {
      setActionError(error.code === '42501' ? 'Нет прав для назначения' : error.message);
    } else {
      setAssignTo('');
      setSelected(null);
      fetchIncidents();
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            >
              {STATUS_OPTIONS.map((s) => (
                <option key={s} value={s}>{s ? statusLabels[s] : 'Все статусы'}</option>
              ))}
            </select>
            <select
            ...
            >
              {SEVERITY_OPTIONS.map((s) => (
                <option key={s} value={s}>{s ? severityLabels[s] : 'Все приоритеты'}</option>
              ))}
            </select>
            <select
              value={filterType}
            ...
            >
              {TYPE_OPTIONS.map((t) => (
                <option key={t} value={t}>{t ? typeLabels[t] : 'Все типы'}</option>
              ))}
            </select>
            {filterActive && (
              <button
        ...
        {loading ? (
          ...
          <div className="rounded-lg border border-border bg-card p-10 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              {filterActive ? 'Попробуйте изменить фильтры.' : 'Очередь пуста.'}
            </p>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-border">
                {incidents.map((inc) => (
                  <tr key={inc.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <Badge variant={severityVariants[inc.severity]}>{severityLabels[inc.severity]}</Badge>
                    </td>
                    <td className="px-4 py-3">
                      <Badge variant={statusVariants[inc.status]}>{statusLabels[inc.status]}</Badge>
                    </td>
                    <td className="px-4 py-3 text-xs text-muted-foreground">
                      {inc.assigned_to ? <span className="font-mono">{inc.assigned_to.slice(0, 8)}…</span> : '—'}
                    </td>
                    <td className="px-4 py-3 text-xs text-muted-foreground">
                    ...
                    <td className="px-4 py-3 text-right">
                      <button
                        onClick={() => { setSelected(inc); setAssignTo(inc.assigned_to ?? ''); setActionError(null); }}
          ...
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/50">
            <div className="w-full max-w-lg rounded-xl border border-border bg-card shadow-2xl p-6 space-y-4">
              ...
              {selected.description && (
                <p className="text-xs text-muted-foreground bg-muted rounded p-2">{selected.description}</p>
              )}
