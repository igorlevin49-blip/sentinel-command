...
interface SLARule {
  id: string; org_id: string; name: string; incident_type: string | null;
  severity: string | null; response_time_s: number; resolve_time_s: number;
...
interface FormState {
  name: string; org_id: string; incident_type: string; severity: string;
  response_time_s: string; resolve_time_s: string; is_active: boolean;
}

const emptyForm: FormState = { name: '', org_id: '', incident_type: '', severity: '', response_time_s: '900', resolve_time_s: '3600', is_active: true };
...
export default function SLARules() {
  ...
  const { isVisible: showForm, show: showNewForm, hide: hideNewForm } = useFormVisibility('newSLA');
  const { isOpen: isEditing, openId: editId, open: startEditById, close: cancelEditUrl } = useDrawerState('editSLA');
  const [form, setForm, clearDraft] = useFormDraft<FormState>('sla-form', emptyForm);
  ...
  const fetchRules = useCallback(async () => {
    ...
    if (error) setError(error.code === '42501' ? 'Нет доступа к SLA правилам' : error.message);
  ...
  function startEdit(r: SLARule) {
    startEditById(r.id);
    setForm({ name: r.name, org_id: r.org_id, incident_type: r.incident_type ?? '', severity: r.severity ?? '', response_time_s: String(r.response_time_s), resolve_time_s: String(r.resolve_time_s), is_active: r.is_active });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const resp = parseInt(form.response_time_s); const resolve = parseInt(form.resolve_time_s);
    if (isNaN(resp) || isNaN(resolve)) { setSaveError('Неверное время (сек)'); setSaving(false); return; }
    const payload = { name: form.name.trim(), incident_type: form.incident_type || null, severity: form.severity || null, response_time_s: resp, resolve_time_s: resolve, is_active: form.is_active };
    let err;
    if (editId) { ({ error: err } = await supabase.from('sla_rules').update(payload).eq('id', editId)); }
    else {
  ...
  async function toggleActive(id: string, current: boolean) {
    await supabase.from('sla_rules').update({ is_active: !current }).eq('id', id); fetchRules();
  }
      ...
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => { showingForm ? cancelForm() : showNewForm(); }}
            ...
            <Plus className="h-4 w-4" /> Добавить SLA
          </button>
        </div>
          ...
          <form onSubmit={handleSubmit} className="rounded-lg border border-border bg-card p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div>
                ...
                <Input value={form.name} onChange={(e) => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Быстрая реакция на тревогу" />
              </div>
              {!editId && (
                <div>
              ...
              <div>
                ...
                <Input type="number" min="1" value={form.response_time_s} onChange={(e) => setForm(f => ({ ...f, response_time_s: e.target.value }))} />
              </div>
              <div>
                <label className="text-xs text-muted-foreground mb-1 block">Время закрытия (сек) *</label>
                <Input type="number" min="1" value={form.resolve_time_s} onChange={(e) => setForm(f => ({ ...f, resolve_time_s: e.target.value }))} />
              </div>
              <div className="flex items-center gap-2">
                <input type="checkbox" id="sla_active" checked={form.is_active} onChange={(e) => setForm(f => ({ ...f, is_active: e.target.checked }))} className="h-4 w-4 rounded border-input" />
              ...
              <tbody className="divide-y divide-border">
                {rules.map((r) => (
                  <tr key={r.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-5 py-3"><span className="font-mono text-xs font-medium text-warning">{formatSeconds(r.response_time_s)}</span></td>
                    <td className="px-5 py-3"><span className="font-mono text-xs font-medium text-muted-foreground">{formatSeconds(r.resolve_time_s)}</span></td>
