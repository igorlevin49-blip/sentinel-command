...
interface EscalationRule {
  id: string; org_id: string; name: string; trigger_after_s: number;
  action: EscalationAction; target_role: string | null; is_active: boolean; created_at: string;
}

const actionLabels: Record<EscalationAction, string> = { notify: 'Уведомить', reassign: 'Переназначить', page: 'Экстренный вызов', close: 'Закрыть' };
const actionVariants: Record<EscalationAction, 'outline' | 'warning' | 'destructive' | 'secondary'> = { notify: 'outline', reassign: 'warning', page: 'destructive', close: 'secondary' };
...
interface FormState {
  name: string; org_id: string; trigger_after_s: string; action: EscalationAction; target_role: string; is_active: boolean;
}

const emptyForm: FormState = { name: '', org_id: '', trigger_after_s: '600', action: 'notify', target_role: '', is_active: true };
...
export default function EscalationRules() {
  ...
  const { isVisible: showForm, show: showNewForm, hide: hideNewForm } = useFormVisibility('newEscalation');
  const { isOpen: isEditing, openId: editId, open: startEditById, close: cancelEditUrl } = useDrawerState('editEscalation');
  const [form, setForm, clearDraft] = useFormDraft<FormState>('escalation-form', emptyForm);
  const formIsDirty = JSON.stringify(form) !== JSON.stringify(emptyForm);
  useUnsavedChangesWarning(formIsDirty);

  const fetchRules = useCallback(async () => {
    setLoading(true);
    const { data, error } = await supabase.from('escalation_rules').select('*').order('trigger_after_s', { ascending: true });
    if (error) setError(error.code === '42501' ? 'Нет доступа к правилам эскалации' : error.message);
  ...
  function startEdit(r: EscalationRule) {
    startEditById(r.id);
    setForm({ name: r.name, org_id: r.org_id, trigger_after_s: String(r.trigger_after_s), action: r.action, target_role: r.target_role ?? '', is_active: r.is_active });
    hideNewForm();
    setSaveError(null);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const payload = { name: form.name.trim(), trigger_after_s: triggerSec, action: form.action, target_role: form.target_role.trim() || null, is_active: form.is_active };
    let err;
    if (editId) { ({ error: err } = await supabase.from('escalation_rules').update(payload).eq('id', editId)); }
    else {
      ...
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => { showingForm ? cancelForm() : showNewForm(); }}
            ...
            <Plus className="h-4 w-4" /> Добавить эскалацию
          </button>
        </div>
          ...
          <form onSubmit={handleSubmit} className="rounded-lg border border-border bg-card p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div>
                ...
                <Input value={form.name} onChange={(e) => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Эскалация при просрочке реакции" />
              </div>
              {!editId && (
                <div>
              ...
              <div>
                ...
                <Input type="number" min="1" value={form.trigger_after_s} onChange={(e) => setForm(f => ({ ...f, trigger_after_s: e.target.value }))} />
              ...
              <div>
                ...
                <select value={form.action} onChange={(e) => setForm(f => ({ ...f, action: e.target.value as EscalationAction }))}
                  className="w-full h-10 rounded-md border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring">
                  {ACTION_OPTIONS.map((a) => <option key={a} value={a}>{actionLabels[a]}</option>)}
                </select>
              </div>
              <div>
                <label className="text-xs text-muted-foreground mb-1 block">Целевая роль (target_role)</label>
                <Input value={form.target_role} onChange={(e) => setForm(f => ({ ...f, target_role: e.target.value }))} placeholder="dispatcher, chief, platform_admin…" />
              </div>
              <div className="flex items-center gap-2 self-end">
                <input type="checkbox" id="esc_active" checked={form.is_active} onChange={(e) => setForm(f => ({ ...f, is_active: e.target.checked }))} className="h-4 w-4 rounded border-input" />
              ...
              <tbody className="divide-y divide-border">
                {rules.map((r) => (
                  <tr key={r.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-5 py-3"><span className="font-mono text-xs text-warning">{formatSeconds(r.trigger_after_s)}</span></td>
                    <td className="px-5 py-3"><Badge variant={actionVariants[r.action]}>{actionLabels[r.action]}</Badge></td>
                    <td className="px-5 py-3 text-xs text-muted-foreground">{r.target_role ?? '—'}</td>
                    <td className="px-5 py-3"><Badge variant={r.is_active ? 'success' : 'secondary'}>{r.is_active ? 'Активно' : 'Откл.'}</Badge></td>
